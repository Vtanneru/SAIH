...
The arithmetic intensity $\mathcal{I}(M)$ is modeled as $\mathcal{I}(M) = \mathcal{I}_0 \cdot (M/M_0)^{0.45}$, where $\mathcal{I}_0 = 2.0$~FLOP/byte at $M_0 = 10$M parameters. This sub-linear (0.45) exponent reflects diminishing memory-reuse gains for larger models, consistent with A100 GPU profiling \cite{Jia2018}. Table~\ref{tab:model-params} summarizes all model parameters and their calibration basis.

\begin{table}[t]
\centering
\caption{Physics-Based Model Parameters}
\label{tab:model-params}
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}} llll @{}}
\toprule
\textbf{Parameter} & \textbf{Value} & \textbf{Role} & \textbf{Calibration Basis} \\
\midrule
$T_\text{peak}$ & 50 TFLOPS & Per-node peak & NVIDIA A100 (bf16) \\
$\beta$ & 1000 GB/s & HBM bandwidth & A100 HBM2e spec \\
$B_\text{net}$ & 200 GB/s & Interconnect & NDR InfiniBand \\
$\alpha$ & 0.15 & Allreduce base cost & $\approx$13\% overhead at 2 nodes \\
$\gamma$ & 0.005 & Congestion term & Yields 55\% overhead at 128 nodes \\
$\delta$ & 0.02 & I/O cost per TB/node & Lustre PFS at NERSC \cite{Byna2020} \\
$\mathcal{I}_0$ & 2.0 FLOP/byte & Base arith.\ intensity & A100 small-model profiling \cite{Jia2018} \\
$\theta$ & 0.45 & Intensity exponent & Empirical GPU scaling \cite{Jia2018} \\
\bottomrule
\end{tabular*}
\end{table}
...
Noise Model & 3\% Gaussian run-to-run variation (per-seed) \\
Repetitions per Config & 5 noise-sampled runs (mean $\pm$ std reported) \\
\bottomrule
\end{tabular*}
\end{table*}
...
\begin{figure}[t]
    ...
    \caption{Strong Scaling Speedup: (a) Predicted speedup vs.\ ideal linear scaling on log-log axes, showing divergence beyond 8 nodes. Maximum speedup of $57\times$ at 128 nodes. (b) Parallel efficiency degradation from 100\% to 44.6\%, with $\pm1\,\sigma$ uncertainty band from 5 noise-sampled runs. The 50\% efficiency threshold is crossed between 32 and 64 nodes. Green region indicates efficient scaling regime.}
...
Validation compares efficiency curves (parallel efficiency \% at matched node counts) between SAIH predictions and MLPerf-HPC reference data, placing both on the same 0--100\% scale and avoiding scale conflation across different hardware systems. The Pearson correlation between SAIH and CosmoFlow efficiency curves is $r=0.947$, and between SAIH and DeepCAM is $r=0.937$. Mean absolute error is 7.3 percentage points (CosmoFlow) and 9.1 percentage points (DeepCAM), indicating that SAIH captures the fundamental shape of the efficiency degradation curve.
...
\begin{figure}[t]
    ...
    \caption{MLPerf-HPC Validation: (a) Scatter plot of predicted vs.\ measured parallel efficiency (\%), with regression line ($r=0.947$) and node-count annotations. Points near the diagonal indicate close agreement. (b) Efficiency curves for predicted, CosmoFlow, and DeepCAM across 1--128 nodes, showing consistent regime transition near 32 nodes. MAE: 7.3 pp (CosmoFlow), 9.1 pp (DeepCAM).}
...
SAIH achieves $r=0.947$ (CosmoFlow) and $r=0.937$ (DeepCAM) on efficiency curves at matched node counts, with mean absolute errors of 7.3 and 9.1 percentage points respectively. These metrics are computed on a common scale (parallel efficiency \%), avoiding the trivially high correlations that arise from comparing two monotonically increasing throughput series on different hardware scales. The agreement is not uniform: SAIH underestimates efficiency at moderate scales (4--32 nodes) where application-specific communication-computation overlap provides benefits not captured by the generic $\eta_{\text{comm}}$ model. At 128 nodes, SAIH predicts slightly higher efficiency (44.6\%) than CosmoFlow achieves (39.2\%), suggesting that extreme-scale collective communication patterns introduce additional overhead beyond the quadratic congestion term.

These systematic deviations highlight an important limitation: SAIH provides a \textit{workload-independent baseline} rather than application-specific predictions. Real workloads may deviate due to framework-specific optimizations (e.g., gradient compression, asynchronous communication), workload-specific memory access patterns, or hardware-software co-optimization. The $r \approx 0.94$ correlation confirms that SAIH captures the correct trend shape---the communication-saturation transition and the efficiency degradation rate---which is its primary design goal.
...
A key question for any physics-based model with manually set parameters is: how sensitive are the key conclusions to parameter choice? Figure~\ref{fig:sensitivity} shows the effect of $\pm$25\% perturbation of $\alpha$ and $\gamma$ on strong-scaling efficiency across 1--128 nodes. The main finding---efficiency below 50\% at 128 nodes---is robust to both perturbations: even with $\alpha$ reduced by 25\%, efficiency at 128 nodes remains below 55\%; with $\alpha$ increased by 25\%, it falls to $\approx$38\%. The 50\% efficiency crossover consistently occurs between 32 and 64 nodes across all parameter variants. The $\gamma$ congestion term has a smaller effect at low node counts but becomes significant beyond 64 nodes, where it controls the steepness of the degradation. These results confirm that the qualitative findings (communication bottleneck at $\geq$16 nodes, efficiency cliff at $>$32 nodes) are robust to $\pm$25\% parameter uncertainty, while the precise efficiency values carry $\pm$5--8 percentage point uncertainty at extreme scale.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figs/Fig9.pdf}
    \caption{Sensitivity Analysis: Effect of $\pm$25\% perturbation of (a) $\alpha$ (base allreduce cost) and (b) $\gamma$ (congestion term) on strong-scaling parallel efficiency. Shaded band shows the sensitivity range; solid line is the baseline. Key findings---efficiency below 50\% at 128 nodes and crossover between 32--64 nodes---are robust to parameter uncertainty.}
...
Through structured scaling experiments (5 noise-sampled runs per configuration), SAIH demonstrates that AI performance on HPC systems exhibits distinctly non-linear behavior. Increasing model complexity improves throughput from 10.7~TFLOPS (10M parameters) to 86.4~TFLOPS (1B parameters, an $8.1\times$ improvement) through increased arithmetic intensity. Data scaling reveals $<$3\% throughput variation across a $64\times$ dataset size range, confirming compute-bound behavior. Strong scaling achieves $57\times$ speedup at 128 nodes with efficiency degrading from 100\% to 44.6\%, with uncertainty bands of $\pm$1--2 percentage points. Weak scaling efficiency follows a similar trajectory (42.8\% at 128 nodes). Cross-dimensional analysis reveals that larger models maintain higher utilization at scale, suggesting co-optimization strategies. Sensitivity analysis confirms that the efficiency crossover below 50\% at 32--64 nodes is robust to $\pm$25\% parameter perturbation.

Validation against MLPerf-HPC benchmarks on efficiency curves yields $r=0.947$ (CosmoFlow) and $r=0.937$ (DeepCAM), with mean absolute errors of 7.3 and 9.1 percentage points, attributable to application-specific communication-computation overlap not captured by the generic model. The interpretable bottleneck classifier correctly identifies the transition from compute-bound ($\leq$8 nodes) to communication-bound ($\geq$16 nodes) regimes, with Number of Nodes and Communication Cost as the two most important features (combined importance: 55.4\%).
